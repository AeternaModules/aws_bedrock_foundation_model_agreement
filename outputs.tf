output "bedrock_foundation_model_agreements_id" {
  description = "Map of id values across all bedrock_foundation_model_agreements, keyed the same as var.bedrock_foundation_model_agreements"
  value       = { for k, v in aws_bedrock_foundation_model_agreement.bedrock_foundation_model_agreements : k => v.id if v.id != null && length(v.id) > 0 }
}
